...
public class DeleteTicketTypeHandler(IUnitOfWork unitOfWork) : IRequestHandler<DeleteTicketTypeCommand, Result>
{
    public async Task<Result> Handle(DeleteTicketTypeCommand request, CancellationToken cancellationToken)
    {
        ...
        if (checkingTicketType.TicketTypeShows.FirstOrDefault()?.Show?.Event is null) return Result.Error("This Ticket Type is not associated with any Event");
        if (checkingTicketType.TicketTypeShows.FirstOrDefault()?.Show?.Event?.Status == EventStatusEnum.Published) return Result.Error("Its event has been published, you can't delete it");
        if (checkingTicketType.TicketTypeShows.FirstOrDefault()?.Show?.Event?.Status == EventStatusEnum.Canceled) return Result.Error("Its event has been canceled, you can't delete it");
        if (checkingTicketType.TicketTypeShows.FirstOrDefault()?.Show?.Event?.Status == EventStatusEnum.Completed) return Result.Error("Its event has been completed, you can't delete it");
